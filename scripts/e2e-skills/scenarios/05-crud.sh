...
scenario_crud() {
	...
	cp "${FIXTURES}/batch-two-skills/skill-a/SKILL.md" "${batch_root}/skill-a-${E2E_RUN_ID}/"
	cp "${FIXTURES}/batch-two-skills/skill-b/SKILL.md" "${batch_root}/skill-b-${E2E_RUN_ID}/"

	if port_skills create "${batch_root}" --publish >/dev/null 2>&1; then
