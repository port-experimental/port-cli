...
func TestExportFromSource_ActionPermissionsNotCollectedWhenExcluded(t *testing.T) {
	...
	// --include actions only — action-permissions should NOT be fetched
	...
	if actionPermsHit {
		t.Error("action permissions endpoint should not be called when --include excludes action-permissions")
	}
}
...
func TestExportFromSource_ActionPermissionsFetchFailureRecordsWarning(t *testing.T) {
	...
	if len(data.Warnings) == 0 {
		t.Error("expected a warning for failed action permissions fetch, got none")
	}
}
